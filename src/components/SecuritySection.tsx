...
export function SecuritySection({ serverId }: { serverId: string }) {
  ...
  return (
    <Card
      title="Security"
      icon="shield"
      description="Anti-cheat advice and suspicious-behaviour flags."
      right={
    ...
    >
      ...
      {advice && !advice.supported && (
        <div className="mb-2 text-2xs text-ink-faint">
      ...
      <div className="border-t border-line pt-2">
        ...
        {sus == null ? (
          <div className="text-2xs text-ink-faint">Reading logs…</div>
        ) : sus.length === 0 ? (
          <div className="text-2xs text-ink-faint">Nothing flagged.</div>
        ) : (
          <ul className="space-y-1">
            {sus.map((s) => (
              <li key={s.name} className="rounded bg-surface-2">
                <button
                ...
                {open === s.name && s.samples.length > 0 && (
                  <pre className="border-t border-line px-2 py-1.5 text-[10px] leading-relaxed text-ink-faint">
                    {s.samples.join("\n")}
                  </pre>
                )}
              </li>
